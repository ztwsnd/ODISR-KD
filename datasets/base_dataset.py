...
class ParentDataset(data.Dataset):
    ...
    def _get_imagepath_list(self, hr_dir, lr_dir, scale, ext='jpg'):
        hr_paths = sorted(
        ...
        # print(len(hr_paths), len(lr_paths), '0000000000000000')
...
class BaseDatasetRAM(ParentDataset):
    ...
    def _initialize(self):
        ...
        # print(hr_dir, lr_dir,'222222222222')
        hr_bin_dir, lr_bin_dir = self._get_hr_and_lr_binary_dir(base_dir)

        hr_paths, lr_paths = self._get_imagepath_list(hr_dir, lr_dir, scale, 'jpg')
        # print(len(hr_paths), len(lr_paths), '0000000000000000')
        ...
        # print(len(self.lr_images),len(self.hr_images), '11111111111111111111')
    ...
    def __getitem__(self, idx):
        ...
        # print(lr.shape, hr.shape, '99999999999')
        pair = self.get_patch(lr, hr)
        pair = set_channel(*pair, n_channels=self.n_colors)
        pair_t = np2Tensor(*pair, rgb_range=self.rgb_range)
        # print(pair_t[0].shape, pair_t[1].shape, '8888888888888')
        return pair_t[0], pair_t[1], filename
...
class BaseDataset(ParentDataset):
    ...
    def _initialize(self):
        ...
        scale = self.scale
        hr_dir, lr_dir = self._get_hr_and_lr_img_dir(base_dir)
        # print(hr_dir, lr_dir, '0000000000000000000000')
        hr_bin_dir, lr_bin_dir = self._get_hr_and_lr_binary_dir(base_dir)
